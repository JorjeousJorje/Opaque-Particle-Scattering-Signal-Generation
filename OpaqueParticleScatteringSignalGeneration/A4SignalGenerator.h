...
public:
	explicit A4SignalGenerator(const TwoOrderParameters& iParams, const LaserParticleParameters& iLPParams)
	:	TwoOrdersSignalGenerator{ iParams , iLPParams }
	{
		...
		if (!mods.count(ScatteringMode::P0)) {
			...
			std::cout << "<A4SignalGenerator>: there is no " + modeString + "mode, possibly result signal is wrong!" << std::endl;
		}
